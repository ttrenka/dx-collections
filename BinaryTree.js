define(["./main"], function(main){
	var BinaryTree=function(data){
		...
		this.forEach = function(fn, obj){
			var a = [];
			inorderTraversalBuildup(root, a);
			main.forEach.call(a, fn, obj);
		};
		this.getIterator=function(){
			var a=[];
			inorderTraversalBuildup(root, a);
			return new main.Iterator(a);
		};
		this.search=function(data){
			return searchHelper(root, data);
		};
		this.toString=function(order, sep){
			if(!order){ order=BinaryTree.TraversalMethods.Inorder; }
			if(!sep){ sep=","; }
			var s="";
			switch(order){
				case BinaryTree.TraversalMethods.Preorder:
					s=preorderTraversal(root, sep);
					break;
				case BinaryTree.TraversalMethods.Inorder:
					s=inorderTraversal(root, sep);
					break;
				case BinaryTree.TraversalMethods.Postorder:
